...
import nicelib.util.multipart.MultipartParser;
import nicelib.util.multipart.Part;
import nicelib.util.multipart.FilePart;
import nicelib.util.multipart.ParamPart;
import nicelib.util.multipart.FileRenamePolicy;
import nicelib.util.multipart.DefaultFileRenamePolicy;
...
public class MultipartRequest {
   ...
   * @param saveDirectory the directory in which to save any uploaded files.
   * @param maxPostSize the maximum size of the POST content.
   * @param policy the encoding of the response, such as ISO-8859-1
   * @exception IOException if the uploaded content is larger than 
   * <tt>maxPostSize</tt> or there's a problem reading or parsing the request.
   */
  public MultipartRequest(HttpServletRequest request,
  ...
  public String[] getParameterValues(String name) {
    try {
      ...
      for (int i = 0; i < count; i++) {
        encodedValues[i] = filter(valuesArray[i]);
      }
