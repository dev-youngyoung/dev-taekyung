...
//JB우리캐피탈 E2E인증서
var cert = "-----BEGIN CERTIFICATE----------END CERTIFICATE-----";
...
var fnInitechCallback;
var bEnc2 = false;

var tekOption = {
	"runtype": TOUCHEN_RUNTYPE,	
    "pki": "TouchEnkeyEx",
    "keyboardonly": "false",
    "defaultenc": "false",
    "verify": "0",
	...
	"driverexcept": "0",   //v1.0.0.41 추가
	"delayedck": "false",  //v1.0.0.41 추가 
	"shiftbypass": "true", //v1.0.0.41 추가
	"allowdup": "false", //v1.0.0.41 추가
	"searchformname":"",
	"tk_isRunningSecurity" : "false"
};
/*
if(TOUCHENEX_UTIL.getOSInfo().name == "Sierra")
{
...
function TK_Init_callback(result) {
	if(result.isvm == "true"){
		TK_processingbar(false);
		if(typeof TK_initSucuess=="function")	TK_initSucuess();
	}
    if (result.result == "true") {
        ...
        try {
			...
			if(result.tkmainver != undefined && result.tkappiver != undefined && result.tkappmver != undefined){
	            	var rtmp1 = true;
	            	var rtmp2 = true;
	            	rtmp1 = TOUCHENEX_UTIL.diffVersion(result.tkappiver,nxKey.tkappiver);
	            	rtmp2 = TOUCHENEX_UTIL.diffVersion(result.tkappmver,nxKey.tkappmver);
	            	if(!rtmp1 || !rtmp2){
			        	//업데이트
	            		TK_installpage();
	            	}
...
/* 20170414 CrossCert HTML5*/
function TK_GetEncCrossCert(frmName, eleId, pubKey, callback)
{
	if(tekOption.idbase){
		frmName = "";
	}
 ...
 * HTML5 연동
 */
function TK_Rescan_HTML(frmObj,eleName) {
	if (bInit == undefined && bInit == 0) return;
	var frmObjName = "";
	if (!tekOption.idbase) {
		frmObjName = frmObj.getAttribute('name');	
	}
	if(!useTouchEnnxKey) return;
		try {
          if (getTouchEnNXEType.MSIEBrowser) {
        	  TK_Request2("EnqueueList", frmObjName, eleName, "");
          } else if (getTouchEnNXEType.WebKitBrowser) {
			    if (tekOption.idbase == "true") {
					makeEncDataId();
				}else{
