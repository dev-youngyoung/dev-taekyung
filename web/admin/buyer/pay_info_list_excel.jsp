...
DataSet member= memberDao.find(" member_no = '"+f.get("s_member_no")+"' and (member_type='01' or member_type='03')", "member_no, member_name");
if(!member.next()){
...
if(!f.get("s_sdate").equals(""))
	list.addSearch("a.accept_date", f.get("s_sdate").replaceAll("-","")+"000000", ">=");
if(!f.get("s_edate").equals(""))
	list.addSearch("a.accept_date", f.get("s_edate").replaceAll("-","")+"999999", "<=");
list.addSearch("a.pay_type", f.get("s_pay_type"), "=");
list.setOrderBy("a.accept_date "+(u.request("mode").equals("excel")?"asc":"desc"));
//목록 데이타 수정
...
String title = member.getString("member_name") + "_결제내역("+f.get("s_sdate")+"~"+f.get("s_edate")+")";
...
out.println(p.fetch("../html/buyer/pay_info_list_excel.html"));
return;
