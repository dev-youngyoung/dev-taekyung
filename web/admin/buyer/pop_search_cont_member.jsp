...
String[] code_status = codeDao.getCodeArray("M008");
	System.out.println(code_status.length);
...
list.setTable("tcb_contmaster a , tcb_cust b, tcb_cust c, tcb_useinfo d, tcb_useinfo_add e,tcb_person f");
list.setFields(
		 "   a.cont_no "
		+" , a.cont_chasu "
		+" , a.cont_name "
		...
		+" , b.pay_yn won_pay_yn "
		+" , c.pay_yn su_pay_yn" 
		+" , nvl(e.recpmoneyamt, d.recpmoneyamt)+ (nvl(e.recpmoneyamt, d.recpmoneyamt) *0.1) recp_amt "
		+" , nvl(e.suppmoneyamt, d.suppmoneyamt)+ (nvl(e.suppmoneyamt, d.suppmoneyamt) *0.1) supp_amt"
		+" , d.paytypecd"
		+" , f.user_id client_id"
		);
...
list.addWhere(" a.member_no = e.member_no(+)");
list.addWhere(" a.template_cd = e.template_cd(+)");
list.addWhere(" c.member_no = f.member_no(+) ");
list.addWhere(" 'Y' = f.default_yn(+) ");
...
while(ds.next()){
	...
	//버튼설정
	ds.put("btn_won_pay", ds.getString("won_pay_yn").equals("")&&ds.getString("paytypecd").equals("30"));//건별제인 경우만.
	ds.put("btn_su_pay", ds.getInt("supp_amt")>0);
}


p.setLayout("popup");
//p.setDebug(out);
p.setBody("buyer.pop_search_cont_member");
p.setVar("popup_title","결제업체 검색");
