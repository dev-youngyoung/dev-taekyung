		...
		if (nagreeCnt>0) { // 결제 라인 있는 경우
			...
			if (agree.next()) {
				...
				if (agree_person.next()) {
					...
					String  = p.fetch("../html/mail/cont_cust_sign.html");
					u.mail(agree_person.getString("email"), "[계약 서명 알림] \"" +  _member_name + "\" 업체가 전자계약서 서명을 완료하였습니다.", mail_body);
			...
			if (!template.getString("doc_type").equals("2")) {
				...
				if(!email.equals("")){
					...
					u.mail(email, "[계약 서명 알림] \"" +  _member_name + "\" 업체가 전자계약서 서명을 완료하였습니다.", mail_body);
				}
			}
			// sms 전송
			cust.first();
