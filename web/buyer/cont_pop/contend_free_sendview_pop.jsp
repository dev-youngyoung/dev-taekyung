...
//내부 결재정보 조회
boolean send_able = false;  // 전송 가능 여부
...
while(agreeTemplate.next()){
 agreeTemplate.put("cont_no", u.aseEnc(agreeTemplate.getString("cont_no")));
	if(!agreeTemplate.getString("mod_reason").equals(""))  // 반려사유가 있으면
	{
		agreeTemplate.put("ag_md_date", "반려<br><font size='1'>"+u.getTimeString("yyyy-MM-dd HH:mm:ss", agreeTemplate.getString("ag_md_date")) + "</font>");
	}
	else if(!agreeTemplate.getString("r_agree_person_id").equals("")) // 승인자 아이디가 있는 경우는 확인
	{
		agreeTemplate.put("ag_md_date", "완료<br><font size='1'>"+u.getTimeString("yyyy-MM-dd HH:mm:ss", agreeTemplate.getString("ag_md_date")) + "</font>");
	}

	// 진행상태에 따라 css 색깔 다르게 적용
	if(agreeTemplate.getString("r_agree_person_id").equals("")) // 확인자 아이디가 공백이므로 아직 확인 안한 상태
