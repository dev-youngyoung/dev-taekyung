...
System.out.println("member_no='"+_member_no+"' and usestartday <='"+u.getTimeString("yyyyMMdd")+"' and useendday>='"+u.getTimeString("yyyyMMdd")+"' ");
if( !useinfo.next() )
{
	u.jsErrClose("서비스 이용기간이 종료 되었습니다.\\n\\n나이스다큐 고객센터[02-788-9097]에 문의하세요.");
	return;
}
...
if(!bandinfo.equals("")){
	if(data.getString("sIdno").equals("")){
		...
		DataSet clientMember = memberDao.find(" vendcd = '"+data.getString("sIdno").replaceAll("-", "")+"' ");
		if(!clientMember.next()){
			//신규추가 비회원 추가
			
			DB db = new DB();
			
			String vendcd = data.getString("sIdno").replaceAll("-", "");
		...
		DataSet temp = memberDao.query(
				    ...
				    +"      , '' division                   "
				    +"   from tcb_member a                  "
				    +"  where a.vendcd = '"+data.getString("sIdno").replaceAll("-", "")+"'" 
		);
		if(!temp.next()){
...
f.addElement("cont_name",data.getString("contName"), "hname:'계약명', required:'Y'");
f.addElement("cont_date", u.getTimeString("yyyy-MM-dd", data.getString("contDay")), "hname:'계약일자', required:'Y'");
if(template.getString("stamp_yn").equals("Y")){
	f.addElement("stamp_type", null, "hname:'인지세 납부 대상', required:'Y'");
}
f.addElement("cont_userno", data.getString("contUserNo"), "hname:'계약번호', maxbyte:'40'");
