...
boolean bviewContNo = u.inArray(member_no, new String[]{"20160900378"}); // 소니코리아
...
while(cfile.next()){
	...
	if(cfile.getString("file_ext").toLowerCase().equals("pdf")){
		...
		cfile.put("down_script","contPdfViewer2('"+cfile.getString("cont_no")+"','"+cfile.getString("cont_chasu")+"','"+cfile.getString("cfile_seq")+"')");
	}else{
...
p.setLayout("popup");
//p.setDebug(out);
p.setBody("cont_pop.contend_msign_sendview_pop");
...
p.setVar("btn_urlcopy", u.inArray(member_no, new String[]{"20120200001"}));  // 더블유컨셉만 URL복사 기능
p.setVar("kakao", bIsKakao);
p.setVar("ContManageNo", bviewContNo ? cont_no+"-"+cont_chasu+"-"+cont.getString("true_random"): "");
p.setVar("form_script", f.getScript());
p.setVar("btn_disuse", u.inArray(member_no, new String[]{"20130500619","20150500312","20171101813","20170501348"})&&cont.getString("status").equals("50"));//위메프, 더블유쇼핑, 에스케이스토아, 아워홈, 만 계약폐기 기능 사용 한다.
p.display(out);
%>