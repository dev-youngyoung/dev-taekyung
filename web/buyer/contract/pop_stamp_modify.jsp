...
if(stamp_money.equals("")&&cont.getInt("cont_chasu")==0)// 처음 입력인 경우/ 최초계약만 자동 계산 NH개발 협의 과정에서 변경계약시 자동산출이 혼동을 줄수 있다고함.20170629
{
	...
	System.out.println("stamp_type:"+cont.getString("stamp_type"));
...
if(gap_yn){ // 갑
	...
	if(member_no.equals(_member_no)){
		if(cont.getString("status").equals("50")) {  // 계약완료
			if(stamp.getString("cert_no").equals("")){// 계약 완료 상태에서 인지세를 등록 안했을 경우 최초 1회만 등록 가능 하도록 함.
				btn = true;
			}
		}else{
			btn = true;
...
if(stamp.getString("cert_no").equals("")){
	p.setBody("contract.pop_stamp_modify");
}else{
