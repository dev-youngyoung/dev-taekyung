...
if(u.isPost()&&f.validate()){
    ...
    while(cust.next()){
    	...
    	if(template.getString("template_cd").equals("2019177")&&cust.getString("sign_seq").equals("3")){//CJ푸드빌 업무도읍계약서 예외처리
    		if(cust.getString("email").equals(""))continue;
    		p.clear();
    	...
    	}else if(template.getString("template_cd").equals("2019269")&&cust.getString("sign_seq").equals("2")){//메리츠캐피탈 중고차판매제휴 예외처리
			String linkUrl = request.getRequestURL().substring(0, request.getRequestURL().indexOf("/web/buyer")) + "/"+rtn_url;
			String smsMessage = "메리츠캐피탈 딜러(판매사원) 업무협약이 완료되었습니다. 감사합니다.-나이스다큐";
			smsDao.sendSMS("buyer", cust.getString("hp1"), cust.getString("hp2"), cust.getString("hp3"),  smsMessage);
		}else{
    ...
    u.jsAlertReplace("서명 저장 하였습니다.\\n\\n계약이 완료 되었습니다.\\n\\n완료된 계약건은 계약완료>완료(보낸계약)에서 확인 하실 수 있습니다.","contract_send_list.jsp?"+u.getQueryString("cont_no, cont_chasu"));
    return;
}
