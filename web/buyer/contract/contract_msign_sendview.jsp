...
String[] filePathArr = new String[cfile.size()];
int pathIndex = 0;
while(cfile.next()){
    ...
    filePathArr[pathIndex] = Startup.conf.getString("file.path.bcont_pdf") + cfile.getString("file_path") + cfile.getString("file_name");
    pathIndex++;
}
...
if (u.isPost() && f.validate()) {
	...
	*/
    ...
    cust.first();
    while (cust.next()) {
    	if (cust.getString("member_no").equals(_member_no)) continue;
   		if (!cust.getString("email").equals("")) {
	        ...
	        if (filePathArr != null && filePathArr.length > 0) {
	        	u.mail(cust.getString("email"), "[계약 체결 알림] " + cont.getString("cont_name") + " 계약이 체결 되었습니다.", p.fetch("../html/mail/cont_finish_mail.html"), filePathArr);
	        } else {
	        	u.mail(cust.getString("email"), "[계약 체결 알림] " + cont.getString("cont_name") + " 계약이 체결 되었습니다.", p.fetch("../html/mail/cont_finish_mail.html"));
	        }
   		}
   		String linkUrl = Config.getWebUrl() + "/" + rtn_url;
   		String subject = "농심 전자계약 안내";
   		String message = "[전자계약][농심] " + auth.getString("_MEMBER_NAME") + " 안내\n"
   				+ "["+auth.getString("_MEMBER_NAME")+"] 계약체결이 완료 되었습니다.\n" 
   				+ "*안내*\n"
   				+ "1.수신받은 계약서에 대해 PC에서도 계약서조회가 가능합니다.("+cust.getString("email")+"에서 확인가능)\n"
   		...
   		// 서명완료(농심>을)
   		smsDao.sendKakaoTalk(cust.getString("hp1"), cust.getString("hp2"), cust.getString("hp3"), "ESC-SD-0004", subject, message, message);
    }
    ...
    u.jsAlertReplace("서명 저장 하였습니다.\\n\\n계약이 완료 되었습니다.\\n\\n완료된 계약건은 계약완료>완료(보낸계약)에서 확인 하실 수 있습니다.","contend_send_list.jsp?"+u.getQueryString("cont_no, cont_chasu"));
    return;
}
cont.put("chgSingType","20".equals(cont.getString("status"))&&!template.getString("template_cd").equals("2019177"));
p.setLayout("default");
//p.setDebug(out);
p.setBody("contract.contract_msign_sendview");
...
p.setVar("btn_meritz_return", cont.getString("template_cd").equals("2019269")&&cont.getString("status").equals("30"));
p.display(out);
%>
