...
--%>
<%!
public Map<String, String> _saveContract(HttpServletResponse response, HttpServletRequest request, Util u, Form f,
		String _member_no, Auth auth) {
	Map<String, String> rtnMap = new HashMap<String, String>();
	rtnMap.put("code", "SUCC");
	rtnMap.put("msg", "저장하였습니다");
	rtnMap.put("url", "contract_modify.jsp?" + u.getQueryString());
	try {
		...
		/* 내부 결재정보 조회 */
		DataSet contAgreeDS = new DataObject("tcb_cont_agree")
				.find("cont_no = '" + cont_no + "' and cont_chasu = '" + cont_chasu + "'", "*", "agree_seq");

		/* 계약업체 조회 */
		DataSet custDS = new DataObject("tcb_cust").find(
		...
		DataSet contTemplateAddDS = new DataObject("tcb_cont_template_add")
				.find("template_cd = '" + template_cd + "'");
		...
		System.out.println("111111111111111 2");
		...
		// decodeing 처리 END
System.out.println("111111111111111 3");
		...
		System.out.println("111111111111111 4");
		...
		System.out.println("111111111111111 5");
		// 계약서파일 생성
		...
		// contDao.item("reg_id", auth.getString("_USER_ID")); -- 최초 작성자 정보가 변경되지 않도록
		// 주석처리
		...
		if (agree_cnt > 0) {
			...
			// 서식 결제라인 수와 계약의 결제 라인 수가 같은 경우만 개인 결제 라인 저장 처리 한다.
			if (contAgreeDS.size() == new DataObject("tcb_agree_template")
					.findCount("template_cd = '" + template_cd + "' ")) {
				...
				for (int i = 0; i < agree_cnt; i++) {
					...
					cont_agree_user.item("agree_name", sAgreeName);
		...
		// 업체 저장
		db.setCommand("delete from tcb_cust_sign_img where cont_no = '" + cont_no + "' and cont_chasu = '"
				+ cont_chasu + "' ", null);
		db.setCommand(
		...
		// 연대보증 업체 저장
		for (int i = 1; i <= 20; i++) {
			db.setCommand("delete from tcb_cust where cont_no = '" + cont_no + "' and cont_chasu = '" + cont_chasu
					+ "' and member_no = '" + "0000000000" + i + "'", null);
			String member_name_y = f.get("op_member_name_" + i); // 이름

			if (!member_name_y.equals("")) {
				String vendcd_y = f.get("op_vendcd_cd1_" + i) + f.get("op_vendcd_cd2_" + i)
						+ f.get("op_vendcd_cd3_" + i); // 사업자번호
		...
		if (contTemplateAddDS.size() > 0) {
			...
			while (contTemplateAddDS.next()) {
				if (contTemplateAddDS.getString("mul_yn").equals("Y")) { // 복수
					String[] colVals = f.getArr(contTemplateAddDS.getString("template_name_en"));
					String colVal = "";
					int cnt = colVals == null ? 0 : colVals.length;
					for (int i = 0; i < cnt; i++) {
						colVal += colVals[i] + "|";
					}
					contaddDao.item(contTemplateAddDS.getString("col_name"), colVal);
				} else { // 단수
					contaddDao.item(contTemplateAddDS.getString("col_name"),
							f.get(contTemplateAddDS.getString("template_name_en")));
				}
		...
		if (u.inArray(errCode,
				new String[] { "NO_CONT_DATA", "MAKE_FILE_ERR", "INSERT_FILE_INFO_ERR", "SAVE_ERR" })) {
