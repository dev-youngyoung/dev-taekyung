...
while (contSub.next()) {
	...
	if (contSub.getString("option_yn").equals("A")) {
		...
		if (contSub.getString("option_yn").equals("Y")) {
			// 선택한 양식의 경우 체크 표시해준다.
...
DataSet cust = custDao.query(
		...
		+ "   and a.sign_seq <= 10 "
		+ " order by display_seq asc"); // sign_seq 가 10보다 큰거는 연대보증
if (!cust.next()) {
...
f.addElement("cont_name", cont.getString("cont_name"), "hname:'계약명'");
...
if (u.isPost() && f.validate()) {
	...
	for (int i=0; i<autoFiles.size(); i++) {
		...
		if (temp.getString("gubun").equals("50")) {
			// null:자동생성, 1:자동첨부, 2:필수첨부, 3:내부용
			cfileDao.item("auto_type", "3"); 
		} else {
