...
if(u.isPost() && f.validate()){
	...
	DataSet enc = encDao.find(" b.asse_no = '"+asse_no+"' and b.div_cd = 'E'" , "b.*");
	String encStatus = "";
	if(enc.next()){
	...
	if("90".equals(saveDiv)){
		...
		//p.setVar("resultParam1", "Q");
		//p.setVar("img_url", webUrl+"/images/email/20110620/");
		//p.setVar("ret_url", webUrl+"/web/buyer/");
		p.setVar("server_name", request.getServerName());
		p.setVar("return_url", "web/buyer/cust/assessment_result.jsp?param="+u.aseEnc(asseNo)+"&param1=Q");
		u.mail(email, "[수시평가 결과] " +  assessment.getString("member_name") + " 평가 결과입니다.", p.fetch("mail/assessment_mail.html"));
		u.jsAlertReplace("평가완료 후 결과전송 하였습니다.","assessment_checking_qlist.jsp?"+u.getQueryString());
		return;
	}

	u.jsAlertReplace(message, "assessment_checking_qlist.jsp?"+u.getQueryString());
	return;
}

p.setLayout("default");
//p.setDebug(out);
p.setBody("cust.assessment_q_checkSheet_sumetime");
