...
while(ds.next()){
	if(!ds.getString("s_user_name").equals("")){
		if(ds.getString("s_status").equals("10")){
			if(auth.getString("_USER_ID").startsWith("wm")){
				ds.put("s_status_nm", ds.getString("s_point").equals("")?"<span style='color:red'>[평가대상]</span>":"<span style='color:red'>[평가중]</span>");
			}else{
				ds.put("s_status_nm", ds.getString("s_point").equals("")?"<span style='color:red'>[평가대상]</span>":"<span style='color:red'>평가중 [ "+ds.getString("s_point")+" ]</span>");
			}
		}else{
			ds.put("s_status_nm","<span style='color:blue'>평가완료</font>" );
		}
	}else{
	...
	if(!ds.getString("qc_user_name").equals("")){
		if(ds.getString("qc_status").equals("10")){
			ds.put("qc_status_nm", ds.getString("qc_point").equals("")?"<span style='color:red'>[평가대상]</span>":"<span style='color:red'>평가중 [ "+ds.getString("qc_point")+" ]</span>");
		}else{
			ds.put("qc_status_nm","<span style='color:blue'>평가완료</font>" );
		}
	}else{
...
p.setLayout("default");
//p.setDebug(out);
p.setBody("cust.asse_progress_list");
