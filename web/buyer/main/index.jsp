...
if (sServerName.indexOf("nicedocu.co.kr") > 0) u.redirect("http://www.nicedocu.com/web/buyer/index.jsp"); */

String sLoginUrl = "./login.jsp";

String user_id = "";
String pin = u.getCookie("pin");
if (pin != null && !pin.equals("")) user_id = Security.AESdecrypt(pin);

int noti_cnt = 9;

DataObject boardDao = new DataObject("tcb_board");
//boardDao.setDebug(out);
DataSet noti = boardDao.find(
	"category = 'noti' and open_yn = 'Y' and open_date <= '" + u.getTimeString("yyyyMMdd") + "' ", 
	"*", 
	"open_date desc", 
	noti_cnt);
while (noti.next()) noti.put("open_date", u.getTimeString("yyyy-MM-dd", noti.getString("open_date")));
